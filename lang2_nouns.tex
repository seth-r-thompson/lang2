...
sah[soon]
tę=rosąm[\gsc{prep}=cook]
pít[hold\gbs\gsc{dist}]
ató[grain\gbs\gsc{dist}]
sin[???]
...
egi[just]
matkó[basket\gbs\gsc{dist}]
aczé[two\gbs\gsc{dist}]
sém[will:be\gbs\gsc{dist}]
tę=het[\gsc{prep}=be:at]
...
sayenar-nat[be.ignorant-\gsc{cvb}]
ot-ik[be-\gsc{prox}]
ezzu[\gsc{assoc}]
Kanyi[\gsc{name}]
ez=lar-n[\gsc{prep=expl-prox}]
...
\begin{kaobox}[frametitle=\sc todo:]
Settle on pronominal forms---right now it's \rz{rat} \gsc{1}, \rz{a(f)} \gsc{2}, \rz{sec} \gsc{3c}, and \rz{moc} \gsc{3n}. There's some isogloss map about whether \gsc{2} is \rz{a} or \rz{af}.
\end{kaobox}