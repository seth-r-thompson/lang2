...
This was a weird sentence to puzzle out. The starting point was realizing that the irrealis and the perfective both use the same preposition, \rz{tę}, to introduce the subordinate verb. In theory it'd be easy enough to combine them, except \rz{sem} is transitive\marginnote{Valency is pretty rigid, so \rz{sem} can't omit an object.} and \rz{ot} is not. 

The first thought was to just make \rz{sem} reflexive and thus eliminate its need for an object,\marginnote{\rz{Sem} is one of a small class of verbs whose reflexive is semantically bleached.} but \rz{segi tę} is already idiomatically used for ``let's'', which might confuse this sentence. The second was to try to concoct a construction where the object of \rz{kiray} is also the object of \rz{sem}, but that seemed like a nightmare. So finally I opted to just use \rz{sem} as normal, but allow a rare \tsc{pro}-drop.
